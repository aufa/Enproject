...
class Crc32 extends CryptUtil
{
    ...
    {
        ! is_null($str) && $this->x_crc32_record[$str] = self::hash($str);
    }
    ...
    public static function hash($string)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $result  = $keyState64[($lowPart ^ ord($string[$i])) & 0xFF] ^ $shifted;
        }
        $retval = $instance->x_crc32_record[$key] = $result ^ 0xFFFFFFFF;
        unset($string, $result, $length, $instance);
        return $retval;
    }
    ...
    {
        $retval = end($this->x_crc32_record);
        return "{$retval}";
    }
