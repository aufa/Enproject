 ...
 */
class Header extends Collector
{
     ...
     * @type int
     */
    protected $x_normalization = self::NORMALIZE_ALL;
    ...
    {
        $this->x_normalization = (int) $normalization;
    ...
    {
        if ($this->x_normalization & static::NORMALIZE_TRIM) {
            $key = trim($key);
        }

        if ($this->x_normalization & static::NORMALIZE_DELIMITERS) {
            $key = static::normalizeKeyDelimiters($key);
        }

        if ($this->x_normalization & static::NORMALIZE_CASE) {
            $key = strtolower($key);
        }

        if ($this->x_normalization & static::NORMALIZE_CANONICAL) {
