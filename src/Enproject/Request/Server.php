...
use Aufa\Enproject\Collector\Server as Collector;

/**
 * Server Variable Request Data
 */
class Server extends Singleton
{
    ...
    /**
     * Check server is Running CLI Mode
     *
     * @return boolean true if Use CLI Mode
     */
    ...
    public static function collector()
    {
        if (!self::$Collector instanceof Collector) {
            self::$Collector = new Collector($_SERVER);
        }
    ...
    public static function httpProtocol()
    {
        ...
        if (! $Protocol || ! in_array($Protocol, array('http', 'https'))) {
            $Protocol = 'http';
            if ($Collector->get('HTTP_X_FORWARDED_PROTO') == 'https'
                || $Collector->get('REQUEST_SCHEME') == 'https'
            ) {
                $_SERVER['HTTPS'] = 'on';
                $Collector->replace($_SERVER);
            }

            $s = $Collector->get('HTTPS', false);
            if ($s == 'on' || $s == '1') {
                $Protocol = 'https';
            } elseif ($Collector->get('HTTP_FRONT_END_HTTPS')
                && strtolower($Collector->get('HTTP_FRONT_END_HTTPS')) !== 'off'
            ) {
                $Protocol = 'https';
            }
        }
    ...
    /**
     * Check if current connection use HTTPS
     *
     * @return boolean
     */
    ...
    {
        if (method_exists(static::collector(), $method)) {
            return call_user_func_array(array(static::collector(), $method), $arguments);
        }
